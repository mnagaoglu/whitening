...
% to make parallel processing faster, divide the data structure into trials
% and save them to the solid state drive
temppath = 'C:\\TempData\\';
for i=1:length(data)
    trialfilename = [temppath sprintf('%d.mat',round(i))];
    currentTrial = data(i);
    save(trialfilename,'currentTrial');
end
...
%             grid on;
            ...
            parfor j=1:length(usefulData)

                trialFile = [temppath sprintf('%d.mat',j)];
                temp = load(trialFile);
                currentTrial = temp.currentTrial;
%                 currentTrial = data(usefulData(j));

                newPixelSizeDeg = apertureSizeDeg/imageSize;
                driftShifts = floor(currentTrial.stitchedPosition/newPixelSizeDeg);
                positionShifts = floor(currentTrial.filteredPosition/newPixelSizeDeg);
...
% %                 title('Drifts + Miscrosaccades')
                ...
                % already exist.
                savepath = [fileparts(directory) 'Spectral Analysis Circle'];
                ...
                % problem.
                savefilename = [savepath filesep sprintf('im_%d_group_%d_j_%d.mat',...
                    imageCounter+19,currentTrial.group,j)];
...
% end
% 
% psImagesFilename = [directory 'PSimages_resize_512.mat']; 
% save(psImagesFilename,'powerSpectra','powerSpectra2','sf');
...
try
% delete the temporarily saved files under the solid state drive
for i=1:length(data)
    trialfilename = [temppath sprintf('%d.mat',round(i))];
    delete(trialfilename);
end
catch delerr
    delerr.stack.name
    delerr.stack.line
    fprintf('Error occured while deleting temporary files..file %d\n',i);
end
...
% create the movie array in advance
movArray = zeros(imageSize,imageSize,windowSize,'uint8');
% im = gpuArray(im);
...
%         waitbar(iter/howManyTimes,wb,'Now it''s time to compute 2D PSD.');
        ...
        % do the fft
        st = tic;
        fftMovie = (abs(ifftshift(fftshift(fftn(double(movArray))),3)).^2)/(m*n*k);
        fftMovie = fftMovie(:,:,1:(k/2+1));
        fftMovie(:,:,2:end-1) = 2*fftMovie(:,:,2:end-1);
        fprintf('FFT of the movie took %.1f seconds.\n',toc(st));

        % create empty 2d PS array
        ps2D = zeros(size(fftMovie,3),cx-1);
        st = tic;
        
        % average across orientations
        for j=1:size(fftMovie,3)
            currentFrame = fftMovie(:,:,j);
